...
- **Never register one HTMLBundle under two routes in `serve.ts`.** Serving both spellings that way looks obvious and it *crashes bun's dev server*: `panic(main thread): integer overflow`, a few seconds into concurrent load. It does not reproduce on a single page or a small suite — measured on the full e2e run against a green baseline, 97/99 passing became 39/106 with 65 connection-level failures and a bun crash dump, which reads exactly like the flake in the next paragraph and is not one. `serve.ts` registers the canonical trailing-slash route only and 301s the other spelling from `fetch`, which is also what Pages does in production.
  **That panic was a bun 1.3 bug, and bun 1.4 no longer has it.** The same `integer overflow` reproduced with the route table entirely correct, purely from CONCURRENCY: measured 2026-08-01 on bun 1.3.14 / Windows, `bunx playwright test` at the default worker count (cores/2) crashed the dev server ~20 s in and took the rest of the run down with it, while `--workers=2` finished the identical suite 136/136 clean; bisected by unregistering the newest page and re-running, so it was not about how many entrypoints there are either. Re-measured 2026-08-25 on bun 1.4.0 / Windows: the full suite at the default 8 workers finishes 211 passed / 2 skipped in 82 s with no panic. CI was unaffected throughout (one worker per shard). The single-canonical-route rule above stays for Pages parity, not for the crash — and if a local full run ever collapses into `ERR_CONNECTION_RESET` again, check the route table once and then re-run with `--workers=2` before believing anything is broken.
...
The Open Graph screenshots live in `images/og/` and are **committed**. `bun run og:capture` re-shoots them through Playwright's API under bun. It used to drive the **CLI under node** instead, because `chromium.launch()` hung forever under bun 1.3 — no error, no timeout; bun 1.4 launches, shoots and closes in under a second (measured), so that detour is gone. `bun run e2e` still shells out to `playwright test`, whose host process gains nothing from moving. Its flags (`--light`, `--full-page`, `--width`, `--height`, `--out`) are for looking at variations locally; the defaults are what the meta tags declare, and the script warns when they are overridden.
...
`gtest_discover_tests` also sets `LABELS`, so a single page's suite runs as `ctest --test-dir build-ci -L rolling-blocks` (or `-L shifting-mosaic`, `-L match-three`, `-L logic-grid`). Logic-grid is the exception to *discovery* itself: since 2026-08-25 its binary is registered as four gtest shards (`LG_TEST_SHARDS`, through `GTEST_TOTAL_SHARDS`/`GTEST_SHARD_INDEX`) rather than as ~9000 per-case entries, because under ctest each discovered case cost an 80 ms process start that registered all ~9000 again — 877 of the 1665 CPU-seconds of CI's whole ctest step, against ~250 s of real solver work. `-L logic-grid` still selects the whole suite, one case is the binary with `--gtest_filter`, and CLion's runner is unaffected. In CI the suites now run as parallel jobs off one build (`cpp-build` → `cpp-test` → the `C++ tests` gate, which also caches a green marker so a change that leaves every C++ input untouched skips the whole side); `shiftingMosaicTest37` gets a runner to itself there, having finished 4062nd of 9760 in the old single job — sharing the machine with the spawn storm rather than running alone.

## Profiling under bun

Bun 1.4 writes profiles as Markdown, which reads in a terminal, greps, and pastes into an LLM: `bun --cpu-prof-md <entry.ts>` writes a CPU profile of whatever the script ran (top functions, call tree, per-function detail) when the process exits, and `bun --heap-prof-md <entry.ts>` the heap (top types by retained size, largest objects, GC roots) — both take a `--cpu-prof-dir`/`--heap-prof-dir`. `bun build ./src/pages/match-three-solver/index.html --outdir <scratch> --metafile-md=<file>` explains what a page's bundle is made of (largest modules, entry-point breakdown, dependency chains), which is the quickest way to see whether dev-mode Lit slipped in; note the CLI build does not run `build.ts`'s plugin, so the tiles show as PNG assets there. `--cpu-prof` / `--heap-prof` / `--metafile` write the Chrome DevTools and esbuild formats instead.
