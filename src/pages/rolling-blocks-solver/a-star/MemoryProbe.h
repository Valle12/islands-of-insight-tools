...
// what implements it. Define RB_NO_MALLINFO to fall back if that ever changes.
#ifndef RB_NO_MALLINFO
#include <malloc.h>
#define RB_HAVE_MALLINFO 1
#endif
#elif defined(_WIN32)
...
inline uint64_t liveAllocatedBytes() {
  ...
  if (!GetProcessMemoryInfo(GetCurrentProcess(), asBase, sizeof(pmc)))
    return 0;
