  ...
  while (changed) {
    ...
    for (size_t i = 0; i < n; i++) {
      ...
      for (int d = 0; d < 4 && !canMove; d++) {
        const Position newAnchor = {static_cast<int8_t>(ax + BitGrid::DX[d]),
                                    static_cast<int8_t>(ay + BitGrid::DY[d])};
...
NodeKey DragSolver::coarseSignature(
  ...
  for (uint8_t i = 0; i < n; i++) {
    ...
    d[i] = static_cast<uint16_t>(
        static_cast<uint16_t>(static_cast<uint8_t>(ax) >> 2) << 8 |
        (static_cast<uint8_t>(ay) >> 2));
  }
