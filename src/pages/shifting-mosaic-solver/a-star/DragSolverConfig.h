...
//
// Grouped into three nested structs rather than kept flat: thirty fields in one
// struct is cpp:S1820, and the three groups below are the ones that are already
// switched on and off together. Nothing about any individual knob changed —
// each kept its default, its meaning and the comment explaining it.
...
struct DragSolverConfig {
  ...
  /// The packing guide, and the assembly knobs that only mean anything with it
  /// on — they are enabled and disabled as a set.
  struct Packing {
  ...
  /// The jam and beam specialists' knobs, including the two jamProfile() gates
  /// that decide whether those arms run on a board at all.
  struct Jam {
    ...
    // blockerPenalty on top of the step cost — the only per-state signal
    // with a gradient on 0-cut compact jam boards, where the admissible
    // cut-suffix h is provably near-flat. Inadmissible (like packing.weight);
    // weighted/portfolio arms only. 0 = off (bit-identical search).
    uint8_t guideWeight = 0;
    ...
    // Must move together with densityPct: on 41193 relaxing either alone
    ...
    // tuning detail. roundNodeCap lowers the per-round cap (0 = per-round
    // default); maxElites widens the retained pool, which otherwise
    // collapses onto the six lowest jam values (one lineage) and loses the
    // higher-jam states a ridge crossing has to pass through.
    uint32_t roundNodeCap = 0;
    uint32_t maxElites = 6;
    // Luby-sequence round caps (1,1,2,1,1,2,4,... × roundNodeCap, clamped
  ...
  /// What ends a pass early. All three unwind the same way, so hitting any of
