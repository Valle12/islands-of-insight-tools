...
//
// The board is colored FIRST — a legal coloring is found by local search
// under whatever rules were asked for — and the clues are then read off the
// regions that coloring produced. Every clue is therefore satisfiable
...
// other colorings, so nothing may pin the solver's answer to the generated
...
// the generated coloring, because that coloring is a solution.
...
struct Options {
  ...
  int width = 0;
  int height = 0;
  /// -1 for a random subset of the coloring rules; otherwise a literal mask.
   ...
   * mirror across it — the check reads the coloring and draws nothing.
   */
  int lotus = 0;
  /**
   ...
   * count is read off the coloring, so every roll that fires places a clue
