...
export class ShapeLayer {
  ...
  anchorOf(square: number): number {
    const squares = this.cellSquares(square);
    if (squares.length === 1) return square;

    const xs = squares.map(one => one % this.width);
    const ys = squares.map(one => Math.floor(one / this.width));
