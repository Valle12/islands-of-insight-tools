...
/**
 * The shipped arm set against the real C++ module — the HAND-BUILT half.
 *
 * `PORTFOLIO` is raced via `wasmHarness.ts`, so this races exactly what the
 * page races, and what is asserted is what has to hold whatever a board turns
 * out to be — the harness spells it out. Every board here exists because some
 * format key or engine path crosses the wasm boundary nowhere else, so the
 * suite still means something on a checkout with no captured boards in it;
 * the captured corpus is `wasm.slow.test.ts`, its own CI shard since it
 * outgrew the fast lane.
 *
 * What this fast half must keep is a board deep enough to stress the wasm
 * stack — `deepSearchBoard` — because the recursion that overflowed it was a
