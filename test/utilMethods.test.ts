...
describe("UtilMethods", () => {
  ...
  describe("PositionToIndex", () => {
    ...
    test.each(cases)(
      "index for position ($x, $y)",
      ({ x, y, gridWidth, result }) => {
        expect(positionToIndex(x, y, gridWidth)).toBe(result);
      },
    );

    test("position to index to position consistency", () => {
      const position = { x: 2, y: 4 };

      const index = positionToIndex(position.x, position.y, 5);
      const convertedPosition = indexToPosition(index, 5);

      expect(convertedPosition).toEqual(position);
    });
  });

  describe("IndexToPosition", () => {
    const cases: IndexToPositionTest[] = [
      { index: 0n, gridWidth: 5, result: { x: 0, y: 0 } },
      { index: 1n, gridWidth: 5, result: { x: 1, y: 0 } },
      { index: 5n, gridWidth: 5, result: { x: 0, y: 1 } },
      { index: 17n, gridWidth: 5, result: { x: 2, y: 3 } },
      { index: 24n, gridWidth: 5, result: { x: 4, y: 4 } },
    ];

    test.each(cases)(
      "position for index $index",
      ({ index, gridWidth, result }) => {
        expect(indexToPosition(index, gridWidth)).toEqual(result);
      },
    );

    test("index to position to index consistency", () => {
      const index = 17n;

      const position = indexToPosition(index, 5);
      const convertedIndex = positionToIndex(position.x, position.y, 5);

      expect(convertedIndex).toBe(index);
    });
  });
