...
Three things in `board.ts` are load-bearing:Three things in `board.ts` are load-bearing:
...
**Drawing one cell out of several elements is the reason
`src/util/gridOutline.ts` gained a third helper.** `markGroupJoins` marks a side
whose neighbour is *inside* the group, plus three interior-corner flags; the
stylesheet then squares off the corners, makes those borders transparent, and
hangs a `::before`/`::after` **bridge** across the 6px grid gap. Each seam is
bridged exactly once, by the square left of or above it. Three things there are
load-bearing:

- **The interior border goes transparent rather than being painted over.**
  Painting over works once the bridge is opaque — but `background-color` is
  transitioned for 120 ms, so for that long the bridge is translucent and every
  border under it shows through. The cell would flash apart at its own joins on
  every click.
- **`--cell-rim` exists so the bridge can draw the rim its cell no longer
  does.** `border-color: inherit` would copy the transparent side, which is
  exactly what the bridge must not do.
- **The corner between two gaps needs its own flags.** Where four squares of one
  cell meet, that little square is interior and gets filled (by running the
  right-hand bridge down through it), and the bridge borders that run past it
  stop being outline. Only where all four are in the cell — an L must keep the
  notch at its inner corner. Without `-corner-tr`/`-corner-bl` a 2x2 inside one
  cell comes out with a cross through it.
- **An uncoloured merged cell's rim is SOLID, and the unknown rim colour is
  OPAQUE.** Both are about the edge reading as one straight line. A dashed
  border restarts its dash pattern on every box it is drawn on, and the edge of
  a merged cell is several boxes with no way to control the phase between them —
  dashed, it came out visibly broken exactly where the squares meet. And a
  translucent rim composites with itself into a darker tick wherever the bridge
  covers the mitred corner where a square's rim meets its own transparent joined
  side, so the unknown rim is now mixed against the well behind the grid rather
  than against `transparent`.
- **A merged cell's squares are promoted to their own compositing layer.** An
  absolutely positioned pseudo-element is its own paint layer in Blink, so the
  bridge's 1px border and the square's own are snapped to the device grid
  INDEPENDENTLY even though they sit at identical CSS coordinates — and at a
  fractional device pixel ratio they land differently, which steps the outline
  sideways at every seam. Measured: 1.5 (Windows at 150%) went from stepped to
  uniform; 1.0 and 2.0 are unaffected either way; 1.25 still varies with or
  without. `contain`, `isolation`, `opacity` and a 2D transform were all
  measured and do nothing. Getting 125% too means one element per shape instead
  of bridges, which is a rewrite of that section.
- **The bridges are deliberately NOT `pointer-events: none`.** A pseudo-element
  hit-tests as its originating element, so leaving them live is what makes the
  seam inside a merged cell clickable — otherwise the gaps between its squares
  are dead strips the pointer falls through, and clicking the middle of a tile
  paints nothing.
